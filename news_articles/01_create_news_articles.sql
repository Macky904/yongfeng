...
-- 说明：在原 DDL 基础上有两处改动：
--   1) 补建触发器依赖函数 update_updated_at_column()（原 SQL 漏了会报错）
--   2) 新增 platform / raw_json 两列（区分来源、存原始 API 返回）
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.news_articles (
  ...
  platform           character varying(30)       NULL,   -- 新增：twitter / news / rss / reddit / telegram / gdelt ...
  raw_json           jsonb                       NULL,   -- 新增：存原始 API 返回，便于后续扩字段
...
CREATE INDEX IF NOT EXISTS idx_news_platform      ON public.news_articles USING btree (platform) TABLESPACE pg_default;
